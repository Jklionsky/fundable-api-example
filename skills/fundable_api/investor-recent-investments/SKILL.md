...
1. Search for the investor by url or linkedin to get their ID(s)
2. Use those ID(s) to find companies where the investor participated in the latest round
...
```bash
curl -s "https://www.tryfundable.ai/api/v1/investor/search/?domain=benchmark.com" \
...
```bash
curl -s "https://www.tryfundable.ai/api/v1/investor/search/?name=benchmark" \
  -H "Authorization: Bearer $FUNDABLE_API_KEY"
```
